...
int main()
{
    const char *input = "/home/dinhnambkhn/3dv_tutorial/bin/data/blais.mp4", *cover = "/home/dinhnambkhn/3dv_tutorial/bin/data/blais.jpg";
    ...
    while (true)
    {
        ...
        {
            obj_points.emplace_back(obj_keypoint[m->trainIdx].pt);
        ...
        for (int i : inlier) inlier_mask.at<uchar>(i) = 1;
        cv::Mat image_result;
        ...
        if (inlier_num > min_inlier_num)
        {
            ...
            double rms = cv::calibrateCamera(std::vector<std::vector<cv::Point3f>>(1, obj_inlier), std::vector<std::vector<cv::Point2f>>(1, img_inlier), image.size(), K, dist_coeff, rvecs, tvecs,
        ...
        // Show the image
        cv::String info = cv::format("Inliers: %zu (%lu%%), Focal Length: %.0f", inlier_num, 100 * inlier_num / match.size(), K.at<double>(0));
