...
int main()
{
    ...
    while (true)
    {
        ...
        if (inlier_num > min_inlier_num)
        {
            ...
            auto rms = cv::calibrateCamera(std::vector<std::vector<cv::Point3f>>(1, obj_inlier), std::vector<std::vector<cv::Point2f>>(1, img_inlier), image.size(), K, dist_coeff, rvecs, tvecs,
        ...
        // Show the image
        cv::String info = cv::format("Inliers: %zu (%lu%%), Focal Length: %.0f", inlier_num, 100 * inlier_num / match.size(), f);
