...
void MouseEventHandler(int event, int x, int y, int flags, void* param)
{
    if (event == cv::EVENT_LBUTTONDOWN)
    {
        ...
        auto *points_src = (std::vector<cv::Point> *)param;
        points_src->emplace_back(x, y);
...
int main()
{
    const char* input = "/home/dinhnambkhn/3dv_tutorial/bin/data/sunglok_desk.jpg";
    cv::Size card_size(450, 250);

    // Prepare the rectified points
    std::vector<cv::Point> points_dst;
    points_dst.emplace_back(0, 0);
    points_dst.emplace_back(card_size.width, 0);
    points_dst.emplace_back(0, card_size.height);
    points_dst.emplace_back(card_size.width, card_size.height);
