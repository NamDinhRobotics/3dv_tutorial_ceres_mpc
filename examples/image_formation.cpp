...
int main()
{
    ...
    FILE* fin = fopen("/home/dinhnambkhn/3dv_tutorial/bin/data/box.xyz", "rt");
    ...
    for (size_t i = 0; i < cam_pos.size(); i++)
    {
        ...
        //eigen matrix rotation from euler angle
        Eigen::Matrix3d R_eigen;
        R_eigen = Eigen::AngleAxisd(cam_ori[i].z, Eigen::Vector3d::UnitZ())
            * Eigen::AngleAxisd(cam_ori[i].y, Eigen::Vector3d::UnitY())
            * Eigen::AngleAxisd(cam_ori[i].x, Eigen::Vector3d::UnitX());

        Eigen::Vector3d tc_eigen(cam_pos[i].x, cam_pos[i].y, cam_pos[i].z);
        //cv::hconcat(Rc.t(), -Rc.t() * tc, Rt) to eigen 3x4 matrix
        Eigen::Matrix<double, 3, 4> Rt_eigen;
        Rt_eigen.block<3, 3>(0, 0) = R_eigen.transpose();
        Rt_eigen.block<3, 1>(0, 3) = -R_eigen.transpose() * tc_eigen;
        //Eigen::Matrix<double, 3, 4> P_eigen = K_eigen * Rt_eigen;
        //K_eigen
        Eigen::Matrix3d K_eigen;
        K_eigen << f, 0, cx, 0, f, cy, 0, 0, 1;
        Eigen::Matrix<double, 3, 4> P_eigen = K_eigen * Rt_eigen;
        //Projection matrix
        ...
        cv::Mat P = K * Rt;
        //printf P and P_eigen
        //std::cout << "P = " << P << std::endl;
        //std::cout << "P_eigen = " << P_eigen << std::endl;
        ...
        cv::imshow(cv::format("3DV_Tutorial: Image Formation %zu", i), image);

        FILE* fout = fopen(cv::format("image_formation%zu.xyz", i).c_str(), "wt");
