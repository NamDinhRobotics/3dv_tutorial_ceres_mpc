...
int main()
{
    ...
    double ransac_n_sample = 2;
    double ransac_thresh = 3.0; // 3 x 'data_inlier_noise'
    int data_num = 1000;
    double data_inlier_ratio = 0.5, data_inlier_noise = 1.0;
    //compute ransac_trial flow ransac_thresh
    int ransac_trial = 1000;
    double p = 0.99;//Confidence level
    double w = data_inlier_ratio; //Inlier ratio
    double n = ransac_n_sample;
    double k = log(1 - p) / log(1 - pow(w, n));
    ransac_trial = (int)k;
    ransac_trial = ransac_trial < 50 ? 50 : ransac_trial;
    printf("ransac_trial: %d\n", ransac_trial);
    ...
    for (int i = 0; i < data_num; i++)
    {
        if (rng.uniform(0.0, 1.0) < data_inlier_ratio)
        {
            ...
            data.emplace_back(x, y); // Inlier
        }
        else data.emplace_back(rng.uniform(0.0, 640.0), rng.uniform(0.0, 480.0)); // Outlier
    }
    ...
    for (int i = 0; i < ransac_trial; i++)
    {
        ...
        for (auto & j : data)
        {
            double error = fabs(line(0) * j.x + line(1) * j.y + line(2));
    ...
    for (auto i : data)
    {
        cv::circle(img, i, 1, cv::Scalar(0, 255, 0), 1);
    }
    cv::line(img, cv::Point(0, -best_line[2] / best_line[1]), cv::Point(640, -(best_line[2] + best_line[0] * 640) / best_line[1]), cv::Scalar(0, 0, 255), 2);
    cv::line(img, cv::Point(0, -lsm_line[2] / lsm_line[1]), cv::Point(640, -(lsm_line[2] + lsm_line[0] * 640) / lsm_line[1]), cv::Scalar(255, 0, 0), 2);
    //truth
    cv::line(img, cv::Point(0, -truth[2] / truth[1]), cv::Point(640, -(truth[2] + truth[0] * 640) / truth[1]), cv::Scalar(0, 255, 255), 2);

    //draw ox, oy
    cv::line(img, cv::Point(0, 0), cv::Point(640, 0), cv::Scalar(255, 255, 255), 2);
    cv::line(img, cv::Point(0, 0), cv::Point(0, -480), cv::Scalar(255, 255, 255), 2);
